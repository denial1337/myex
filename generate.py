...
def generate(amount):
    ...
    [
        resolve_order(
            LimitOrder.objects.create(
                price=p, quantity=q, dir="sell", status=0, symbol=symbol
            )
        )
    ...
    [
        resolve_order(
            LimitOrder.objects.create(
                price=p, quantity=q, dir="buy", status=0, symbol=symbol
            )
        )
...
def generate_limit_orders():
    ...
    for depo in Depo.objects.all()[:50]:
        ...
        order = LimitOrder.objects.create(
            initiator=depo, price=price, quantity=quantity, dir=d, symbol=s
        )
        resolve_order(order)
