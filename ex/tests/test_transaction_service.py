...
from services.transaction_service import make_market_transaction, _get_position_by_order
...
class TestTransactionService(TestCase):
    ...
    def test_make_market_transaction(self):
        ...
        maker_pos = _get_position_by_order(lo2)
        taker_pos = _get_position_by_order(mo)

        self.assertEqual(maker_pos.quantity, -1)
        self.assertEqual(taker_pos.quantity, 1)

        mo2 = MarketOrder.objects.create(
            initiator=taker_depo,
            symbol=symbol,
            quantity=1,
            direction=AbstractOrder.OrderDirection.BUY,
        ...
        maker_pos = _get_position_by_order(lo1)
        taker_pos = _get_position_by_order(mo3)
