...
from services.order_validation_service import is_valid_market_order, is_valid_limit_order
...
class TestSymbol(TestCase):
    def test_market_order_validation(self):
        ...
        mo = MarketOrder.objects.create(initiator=taker_depo, symbol=symbol, quantity=6,
                                        dir=AbstractOrder.OrderDirection.SELL)
        self.assertEqual(is_valid_market_order(mo), False)


        mo = MarketOrder.objects.create(initiator=taker_depo, symbol=symbol, quantity=1,
                                        dir=AbstractOrder.OrderDirection.SELL)
        self.assertEqual(is_valid_market_order(mo), True)
        ...
        # buy tests
        taker_depo = Depo.objects.create(start_equity=10000)
        mo = MarketOrder.objects.create(initiator=taker_depo, symbol=symbol, quantity=4,
    ...
    def test_limit_order_validation(self):
        ...
        lo = LimitOrder.objects.create(initiator=taker_depo, symbol=symbol, quantity=1,
                                        dir=AbstractOrder.OrderDirection.SELL, price=500)
        self.assertEqual(is_valid_limit_order(lo), True)


        lo = LimitOrder.objects.create(initiator=taker_depo, symbol=symbol, quantity=6,
                                        dir=AbstractOrder.OrderDirection.SELL, price=500)
        self.assertEqual(is_valid_limit_order(lo), True)
        ...
        lo = LimitOrder.objects.create(initiator=taker_depo, symbol=symbol, quantity=1,
                                        dir=AbstractOrder.OrderDirection.SELL, price=500)
        self.assertEqual(is_valid_limit_order(lo), False)

        # buy tests
        taker_depo = Depo.objects.create(start_equity=10000)
        lo = LimitOrder.objects.create(initiator=taker_depo, symbol=symbol, quantity=4,
                                        dir=AbstractOrder.OrderDirection.BUY, price=501)
        self.assertEqual(is_valid_limit_order(lo), True)
